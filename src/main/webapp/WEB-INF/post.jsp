					...
					<div class="col-md-10 offset-md-1 rounded-top content-panel">
						...
						<div class="row mb-3">
							...
							<div class="col-sm-12" id="newComment">
								<form:form name="newCommentForm" id="newCommentForm" method="post" action="/comment/create" modelAttribute="newComment">
										...
										<c:if test="${currentUser != post.answer.commenter}">
											<c:choose>
												<c:when test="${post.answer.commentLikes.contains(currentUser)}">
													<a href='/comment/<c:out value="${post.answer.id}"/>/like' class="liked"><i class="fas fa-thumbs-up"></i></a>
												</c:when>
												<c:otherwise>
